...
かなりの部分で、*[Gauche*]のAPIを参考にしているので迷ったら*[http://practical-scheme.net/gauche/man/gauche-refj.html|Gaucheのユーザリファレンス*]を参照していただくと解決すると思います(笑)

*[Nendo.Tutorial*] 
*[Nendo.ReferenceManual*] 

今回のポイントはgemライブラリを利用する上で全般的に足りなかった機能が追加されたことでしょう。
0.3.3以後は*[http://fallabs.com/kyotocabinet/|Kyoto Cabinet*]を使った*[Sekka*]という日本語入力メソッドを作りながら足りない部分を補いました。
...
次は、*[http://fallabs.com/kyotocabinet/|Kyoto Cabinet*]のAPIを使う例です。
 *Ruby*
!#!/usr/local/bin/ruby
!# -*- coding: utf-8 -*-
!
